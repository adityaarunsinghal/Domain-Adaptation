...
# this will train MLM from SCRATCH!! (which we don't actually want so don't use this script)
...
tokenizer.train(files=paths[-1], vocab_size=10, min_frequency=2, special_tokens=[
    "<s>",
    "<pad>",
    "</s>",
    "<unk>",
...
tokenizer = ByteLevelBPETokenizer(
    "./movie_roberta/roberta_DAPT_movies_model/vocab.json",
    "./movie_roberta/roberta_DAPT_movies_model/merges.txt",
)
...
tokenizer = RobertaTokenizerFast.from_pretrained("./movie_roberta/roberta_DAPT_movies_model/", max_len=512)

print("-------------MADE ROBERTA TOKENIZER------------")
...
trainer.save_model("./movie_roberta/roberta_DAPT_movies_model/")
...
fill_mask = pipeline(
    "fill-mask",
    model="./movie_roberta/roberta_DAPT_movies_model/",
    tokenizer="./movie_roberta/roberta_DAPT_movies_model/"
)
